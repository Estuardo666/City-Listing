...
import { UserEventsList } from '@/components/features/dashboard/user-events-list'

export const metadata = { title: 'Mis Eventos — Dashboard' }
...
export default async function DashboardEventosPage() {
  ...
  const [total, approved, pending] = await Promise.all([
    ...
    prisma.event.count({ where: { userId: session.user.id, status: 'APPROVED' } }),
    prisma.event.count({ where: { userId: session.user.id, status: 'PENDING' } }),
  ])
      ...
      <section className="mx-auto max-w-4xl space-y-8 px-4 sm:px-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-1">
            ...
            <h1 className="text-2xl font-semibold text-foreground sm:text-3xl">Mis eventos</h1>
            <p className="text-sm text-muted-foreground">Gestiona los eventos que has publicado.</p>
          </div>
        ...
        {total === 0 ? (
          <div className="rounded-2xl border border-dashed border-border/60 bg-card px-6 py-14 text-center">
            ...
            <p className="text-sm font-medium text-muted-foreground">Aún no has creado ningún evento.</p>
