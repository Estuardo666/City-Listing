...
export function ShareButton({ url, title, description, className = '' }: ShareButtonProps) {
  ...
  const items = [
    { label: copied ? 'Copiado' : 'Copiar enlace', icon: copied ? Check : LinkIcon, action: handleCopyLink },
    { label: 'WhatsApp', icon: null, action: handleWhatsApp, wa: true },
    { label: 'Facebook', icon: null, action: handleFacebook, fb: true },
    { label: 'X', icon: null, action: handleTwitter, x: true },
    { label: 'Telegram', icon: null, action: handleTelegram, tg: true },
  ]
        ...
        {open && (
          ...
          >
            {items.map((item) => (
              <button
                key={item.label}
                onClick={() => { item.action(); if (!item.wa && !item.fb && !item.x && !item.tg) setTimeout(() => setOpen(false), 300) }}
                className="flex w-full items-center gap-2.5 px-3.5 py-2.5 text-sm text-foreground transition-colors hover:bg-accent"
              >
