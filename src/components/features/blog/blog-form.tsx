...
export function BlogForm({ categories, initialData, redirectTo = '/blog' }: BlogFormProps) {
  ...
  const onSubmit = async (data: PostInput) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success('Artículo enviado para revisión. Será publicado una vez aprobado.')
        router.push(redirectTo)
        router.refresh()
      } else {
        toast.error(result.error ?? 'Error al crear el artículo')
      }
    } catch {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-4">
          ...
          <Button type="submit" disabled={isSubmitting}>
            {isSubmitting ? 'Enviando artículo...' : 'Enviar artículo'}
          </Button>
        </div>
      </form>
    </Form>
